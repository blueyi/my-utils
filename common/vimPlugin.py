...
with open(vimrc, 'r') as text:
    for tline in text:
        tline = tline.lstrip()
        if len(tline.strip()) != 0 and tline[: tline.find(' ')] == 'Plug' and 'YouCompleteMe' not in tline:
            plugin = tline[tline.find("'") + 1 : tline.find("'", tline.find("'") + 1)]
...
# vim plugged文件夹
if not os.path.exists(userPath() + '/.vim/plugged'):
    run_cmd('mkdir -p ~/.vim/plugged', error_log)
...
# For vim-plug
vimPlug_cmd = 'curl -fLo ~/.vim/autoload/plug.vim --create-dirs \
    https://raw.githubusercontent.com/junegunn/vim-plug/master/plug.vim'

run_cmd(vimPlug_cmd, error_log)

# ---Youcompleteme install---
def installYCM():
    welcomePrint('Configing Youcompleteme')
    ycm = userPath() + '/' + '.vim/plugged/YouCompleteMe'
    os.chdir(ycm)
    run_cmd('git submodule update --init --recursive', error_log)
