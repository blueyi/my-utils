...
- **~/.cursor/projects** → `cursor_bak/projects` (per-workspace state; `terminals/`, `agent-transcripts/`, `mcps/` schema caches, and `canvases/{node_modules,tsconfig.json}` are gitignored — user MCP config is in `User/mcp.json`)
